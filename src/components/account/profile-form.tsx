  ...
  return (
    <form action={action} className="grid gap-4" noValidate>
      ...
      <Field id="fullName" label="Full name" errors={errors.fullName}>
        {(describedBy, invalid) => (
          <Input id="fullName" name="fullName" autoComplete="name" required maxLength={120}
            defaultValue={profile.full_name} aria-describedby={describedBy} aria-invalid={invalid} />
        )}
      </Field>
      <Field id="phone" label="Phone (optional)" errors={errors.phone}
        ...
        {(describedBy, invalid) => (
          <Input id="phone" name="phone" type="tel" autoComplete="tel" inputMode="tel"
            defaultValue={profile.phone ?? ""} aria-describedby={describedBy} aria-invalid={invalid} />
        )}
      </Field>
      <Field id="countryCode" label="Country you live in" errors={errors.countryCode}>
        {(describedBy, invalid) => (
          <NativeSelect id="countryCode" name="countryCode" defaultValue={profile.country_code ?? ""}
      ...
      <Field id="preferredCurrency" label="Show prices in" errors={errors.preferredCurrency}>
        {(describedBy, invalid) => (
          <NativeSelect id="preferredCurrency" name="preferredCurrency" defaultValue={profile.preferred_currency}
            aria-describedby={describedBy} aria-invalid={invalid}>
